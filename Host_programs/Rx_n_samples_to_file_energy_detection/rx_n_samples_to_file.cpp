...
template<typename samp_type> void recv_to_file(
    ...
    size_t num_rx_samps = rx_stream->recv(&buff.front(), buff.size(), md, 3.0, enable_size_map);//receive the samples
    ...
    if (md.error_code != uhd::rx_metadata_t::ERROR_CODE_NONE){
     ...
     outfile.write((const char*)&buff_ed.front(), num_requested_samples*sizeof(ed_type));
...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    std::string file("samples_data.dat");//name of the file to write binary samples to
    std::string type("short");//sample type: double, float, or short
    std::string ant("RX2");
    std::string subdev("A:0");
    std::string ref("internal");
    std::string wirefmt("sc16");
    size_t total_num_samps(103424);//total number of samples to receive
    size_t spb(103424);// samples per buffer
    double rate(10e6);// rate incoming samples
    double freq(400e6);//RF center Frequency
    ...
    desc.add_options()
      ("help", "help message")
      ("file", po::value<std::string>(&file), "RF center frequency in Hz")
      ...
      ("w_sz",po::value<int>(&window_size)->default_value(2),"window size value only power of 2 greater than 8")
    ...
    if (vm.count("help")) {
        ...
        std::cout
            << std::endl
            << "This application receive data from a single channel of a USRP device and stores the data into one binary files with the detection and the square magnitude.\n"
            << std::endl;
        return ~0;
    }
...
#define recv_to_file_args(format)					\
    (usrp, format, wirefmt, file, spb, total_num_samps)
