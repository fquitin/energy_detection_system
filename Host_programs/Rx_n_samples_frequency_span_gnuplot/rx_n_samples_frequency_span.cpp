...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    if (vm.count("help")) {
        ...
        std::cout
            << std::endl
            << "This application streams data from a single channel of a USRP device changing the frequency and analysing the spectrum, plot the result usign GNU Plot but doesn't refresh the plot just take 1024 samples from each span.\n"
            << std::endl;
        return ~0;
    }
    ...
    for( i=0 ; i<N;i++){
      ...
      usrp->set_command_time(cmd_time[i]+3*delta);
      usrp->set_user_register(DC_EL_TC3_ADDRESS,dc_tc[3*i+2],0);
      stream_cmd.time_spec = cmd_time[i]+4*delta;
